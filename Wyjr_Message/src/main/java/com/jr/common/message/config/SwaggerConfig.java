...
public class SwaggerConfig {
    ...
    public ApiInfo createA(){
        ApiInfo info=new ApiInfoBuilder().title("验证码统一消息平台").

                description("实现短信发送信息").build();
        return info;
    }
    /**
     * 创建API应用
    ...
    public Docket createDocket(){
       Docket docket=new Docket(DocumentationType.SWAGGER_2).apiInfo(createA()).select().
               apis(RequestHandlerSelectors.basePackage("com.jr.common.message.core.controller")).
               build();
       return docket;
    }
}