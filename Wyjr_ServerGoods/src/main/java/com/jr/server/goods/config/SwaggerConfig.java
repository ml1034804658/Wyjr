package com.jr.server.goods.config;
...
public class SwaggerConfig {
    ...
    public ApiInfo createA(){
        ApiInfo info=new ApiInfoBuilder().title("我要捡人商品-数据接口平台").
                contact( new Contact("第五组","http://www.mobiletrain.org","xingfei_work@163.com")).
                description("实现一个我要捡人商品的数据接口").build();
        return info;
    }
    /**
     * 创建API应用
    ...
    public Docket createDocket(){
       Docket docket=new Docket(DocumentationType.SWAGGER_2).apiInfo(createA()).select().
               apis(RequestHandlerSelectors.basePackage("com.jr.server.goods.controller")).
               build();
       return docket;
    }
}